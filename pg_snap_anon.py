...
def capture_fdw_config(conn, patch_dir: str) -> None:
    """Query pg catalog for FDW config and write post_restore_fdw.sql."""
    ...
    if not rows:
        out_file.write_text(
            "-- No FDW configuration found on this instance.\n"
            "-- Nothing to restore.\n"
        )
        click.echo("FDW capture: no foreign servers found.")
        return
    ...
    for srvname, fdwname, server_options, mapped_user, _user_options in rows:
    ...
    out_file.write_text("\n".join(lines))
    click.echo(f"FDW config captured → {out_file}")
